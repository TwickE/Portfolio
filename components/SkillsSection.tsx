...
const SkillsSection = ({ backgroundColor }: { backgroundColor: string }) => {
  ...
  const handleMouseEnter = () => {
    // Intentionally empty, handled in handleMouseMove to prevent initial flash
  }
    ...
    <section className={`${backgroundColor} w-full py-12 overflow-hidden`}>
      ...
      >
        {isMouseInSection && isHoverSupported && ( // Glowing orb that follows the mouse
          <span
            className="absolute left-0 top-0 pointer-events-none w-[200px] h-[200px] rounded-full bg-my-primary blur-3xl transition-opacity duration-300"
            style={{
